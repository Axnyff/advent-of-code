...
const canBeRemoved = (brick) => {
  let otherBricks = bricks.filter(b => b !== brick);
  for (let brick2 of otherBricks) {
    let otherBricks2 = otherBricks.filter(b => b !== brick2);
    if (isOnTheFloor(brick2, otherBricks2) !== isOnTheFloor(brick2, [...otherBricks2, brick])) {
      return false;
    }
  }
  return true;
