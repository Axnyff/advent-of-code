...
List<Pair<String, int>> getPossibleMoves(Map<String, String> state, String key) {
  ...
  var roomIsValid =
    ...
    ((state["${room}-3"] ?? amphipod) == amphipod) &&
    ((state["${room}-4"] ?? amphipod) == amphipod)
    ;
  ...
  if (y == 0) {
    ...
    if (canGoToTarget(state, x, room, 0, 4)) {
      return [Pair(getKey(room, 4), (diff.abs() + 4) * getCost(amphipod))];
    }
    if (canGoToTarget(state, x, room, 0, 3)) {
      return [Pair(getKey(room, 3), (diff.abs() + 3) * getCost(amphipod))];
    }
