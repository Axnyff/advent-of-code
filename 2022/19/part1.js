...
const getMaxOutputForBluePrint = (blueprint) => {
  ...
  while (time <= 24) {
    ...
    for (let events of toExplore) {
      ...
      if (events === "WWC") {
        console.log(oreMissing2, blueprint.clay, state.ore);
      }
      const clayTime = Math.ceil(oreMissing2 / state.oreBot);
      if (events.length + clayTime <= 24) {
          add = true;
      ...
      if (state.clayBot) {
        ...
        const obsidianTime = Math.max(Math.ceil(clayMissing / state.clayBot), Math.ceil(oreMissing / state.oreBot));
        if (events.length + obsidianTime <= 24) {
          add = true;
      ...
      if (state.obsidianBot) {
        ...
        const geodeTime = Math.max(Math.ceil(obsidianMissing / state.obsidianBot), Math.ceil(oreMissing / state.oreBot));
        if (events.length + geodeTime <= 24) {
          add = true;
...
// console.log(computeState(blueprint, "WWCWCWCWWWBCWWBWWGWWGWWWG"));
console.log(getMaxOutputForBluePrint(blueprint));
