...
(defn sorting-method
  [a b]
  (let [countA (count a) countB (count b)]
    (or (< countB countA) (and (= countB countA) (< (first a) (first b))))))

(defn gen-checksum
  [letters]
  (let [sorted (sort letters)
        partitioned (partition-by identity sorted)
        good-sorted (sort sorting-method partitioned)
        first-fives (take 5 good-sorted)]
    (apply str (map #(first %) first-fives))))
...
(defn translate-char
  ...
  (if (= c \-)
    \_ 
    (let [n (.indexOf letters-cycle c)
          new-n (+ n id)
          new-n-modded (mod new-n 26)
          new-c (nth letters-cycle new-n-modded)]
      new-c)))
...
(defn decrypt-room
  [letters id]
  (let [mod-id (mod id 26)]
    (apply str (map #(translate-char % mod-id) letters))))

(defn destruct-line
  [line]
  (let [[ignore & [letters id check]] (.match line #"([a-z-]+)-(\d+)\[(\w+)\]")
        filtered-letters (.replace letters (js/RegExp "-" "g") "")
        ]
    [filtered-letters (js/parseInt id 10) check]))

(defn line->code
  [line]
  (let [[letters id check] (destruct-line line)
        gen-check (gen-checksum letters)]
    (if (= gen-check check)
      id
      0)))
...
; (def result
;   (apply + (map line->code lines)))

(def result
  (map 
    (fn [line]
      (let [[letters id] (destruct-line line)]
        [(decrypt-room letters id) id]))
    lines
    ))
(println result)
